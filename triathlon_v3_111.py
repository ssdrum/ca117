...
class Triathlon(object):
    ...
    def best(self):
        results = {k: sum([int(t) for t in v.times.values()]) for (k, v) in self.triathletes.items()}
        best_id = min(results.items(), key=lambda x: x[1])[0]
        return self.triathletes[best_id].__str__()

    def worst(self):
        results = {k: sum([int(t) for t in v.times.values()]) for (k, v) in self.triathletes.items()}
        worst_id = max(results.items(), key=lambda x: x[1])[0]
        return self.triathletes[worst_id].__str__()
